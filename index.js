...
Solis5G.prototype = {
  ...
  _getStatus: function (callback) {
    ...
    this._httpRequest(
      this.useCloud ? this.SOLIS_AUTH.url + resource : this.url,
      body,
      this.useCloud ? 'POST' : 'GET',
      headers,
        ...
        if (error) {
          ...
          try {
            ...
            if (activeAlarms > 0) {
              ...
              let startError = responseBody.indexOf('var webdata_alarm =');
              let endError = responseBody.indexOf('var', startError + 3);

              this.log.warn('Device in alarm mode', responseBody.substring(startError, endError));
              this.service.getCharacteristic(Characteristic.On).updateValue(1);

              if (!switchState) {
                this.log.debug('Notify user about Alarm!');
                // Execute MotionEvent
                ...
                this.log.debug('SwitchState was already On (no need to notify user)');
              }
            }
